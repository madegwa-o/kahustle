...
interface Props {
  seller: PropertyDetail["seller"]
  canView: boolean
}

export default function PropertySeller({ seller, canView }: Props) {
  if (!seller) return null

  return (
    <div className="rounded-xl border border-border bg-card">
      <div className="px-4 py-3 border-b border-border">
        <h2 className="font-semibold text-foreground">Property seller information</h2>
      </div>
      <div className="p-4 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-foreground">{seller.name ?? "Unknown property seller"}</p>
            {seller.location && (
        ...
        {canView ? (
          ...
          <div className="flex items-start gap-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 p-3 text-sm">
            ...
            <div>
              ...
              <p className="text-amber-700 dark:text-amber-500 text-xs mt-0.5">You need the <strong>Property Buyer</strong> role to view seller contact details.</p>
